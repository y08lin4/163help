...
(function() {
    ...
    async function fetchConfig() {
        ...
        return new Promise(r => GM_xmlhttpRequest({
            ...
            headers:{'X-Music-Helper-Version': CURRENT_VERSION, 'X-Client-Type': 'docker', 'X-Vip-Type': String(vt)},
            onload:res=>{
    ...
    // 网易云会员等级（vipType）采集：补齐 docker 端 X-Vip-Type 上报（对齐扩展端，服务端 /api/me、/api/next 消费）。
    ...
    async function buildSignHeaders(method, fullUrl, rawBody, token) {
        ...
        try {
            ...
            // Docker 客户端密钥模式（mh_ck_ 前缀）：HMAC 密钥 = sha256(凭证) 小写 hex
            // （与服务端只存 key 哈希对应；服务端 tokenHash 即 sha256 小写 hex，无盐）。
    ...
    async function claimTicket(ticket) {
        ...
        return new Promise(r => GM_xmlhttpRequest({
            ...
            headers:{'Content-Type':'application/json','X-Music-Helper-Version': CURRENT_VERSION, 'X-Client-Type': 'docker', 'X-Vip-Type': String(vt)},
